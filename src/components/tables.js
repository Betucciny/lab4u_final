import {
    Button, Input, Modal, ModalBody, ModalContent, ModalFooter, ModalHeader, Pagination,
    Table,
    TableBody,
    TableCell,
    TableColumn,
...
function TableExistenciasMateriales({existencias, visible, onRowClick, material}) {
    const headers = ["Cantidad", "Numero de Serie", "Observaciones", "Codigo", "Ubicacion", "Entrada"];
    ...
    return (
        visible ? (
            <Table
                ...
                selectionMode="replace"
                onRowAction={(row) => {
                    const existencia = existencias.filter((existencia) => existencia.codigo.toString() === row)[0];
                    onRowClick(existencia, material)
                }}
                bottomContent={
                    <div className="flex w-full justify-center">
                        <Pagination
                ...
                <TableBody>
                    {items.map((existencia) => (
                        <TableRow key={existencia.codigo}>
                            <TableCell>{existencia.cantidad}</TableCell>
                            <TableCell>{existencia.numserie}</TableCell>
                            <TableCell>{existencia.observaciones}</TableCell>
                            <TableCell>{existencia.codigo}</TableCell>
                            <TableCell>{existencia.ubicacion}</TableCell>
                            <TableCell>{existencia.entrada}</TableCell>
                        </TableRow>
                    ))}
...
function TableMateriales({materiales, visible, onRowClick, onRowClickExistencias}) {

    const headers = ["Nombre", "Marca", "Tipo", "Descripcion", "Existencias"];
                    ...
                    {(onClose) => (
                        <>
                            ...
                            <ModalBody>
                                {
                                    active !== undefined ?
                                        <TableExistenciasMateriales existencias={active} visible={true}
                                                                    onRowClick={onRowClickExistencias} material={material}/> : <></>
                                }
                            </ModalBody>
                            <ModalFooter>
                        ...
                        {items.map((material) => (
                            <TableRow key={material.id}>
                                ...
                                <TableCell> <Button onPress={
                                    () => {
                                        setActive(material.existencias);
        ...
        <>
            <Modal isOpen={isOpen} onOpenChange={onClose} size="4xl">
                <ModalContent>
                    {(onClose) => (
                        <>
                            <ModalHeader>Reactivos</ModalHeader>
                            <ModalBody>
                                {
                                    active !== undefined ?
...
function TableContenidoVale({items, onDelete, onNumberChange}) {
    const headers = ["Nombre", "Tipo", "Codigo/Numero de Serie", "Cantidad", "Eliminar"];
    const max = 10
                    ...
                    <TableRow key={index}>
                        ...
                        <TableCell>  {item.tipo !== "reactivo" ?
                            <Input
                                ...
                                className="w-20"
                                size={"sm"}
                                onChange={(e) => {onNumberChange(index, e)}}/>: "1"}
                        </TableCell>
                        <TableCell> <Button onPress={() => {
                            onDelete(index)
